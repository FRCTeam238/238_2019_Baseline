...
public class CommandController {
    ...
    /*
     * Gets the buttons that are pressed or switches that are set from the controls
     * (joysticks or custom) which the values ( button1 = 1 etc) are the key into a
     * Map of commands that have been pre-loaded in the setup methods, then "get"s
     * the command associated with the key in the hashmap and calls the execute
     * function on that command.
     */
    /*
     * public void buttonPressed(int[] commandValue){
     * 
     * Command commandForTheButtonPressed; Integer buttonPressed =
     * commandValue(CrusaderCommon.INPUT_DRIVER_LEFT_JS);
     * 
     * commandForTheButtonPressed = driverLeftCmdList.get(buttonPressed);
     * if(commandForTheButtonPressed != null){ commandForTheButtonPressed.execute();
     * }
     * 
    ...
    public void joyStickCommandExecution(HashMap<Integer, Integer[]> commandValues) {
         ...
         * 
         * //Because the shooter command takes button inputs in order to switch between
         * a dynamic and static shooter if(buttonPressed[i] == 1) {
         * operatorCommandsForTheButtonPressed[0].execute(1); } else if(buttonPressed[i]
         * == 5) { operatorCommandsForTheButtonPressed[0].execute(5); } else {
